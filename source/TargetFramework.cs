...
public readonly struct TargetFramework : IEquatable<TargetFramework>
{
    ...
    public static bool TryParse(ReadOnlySpan<char> text, out TargetFramework targetFramework)
    {
        targetFramework = new(text);
        if (Array.IndexOf(All, targetFramework) != -1)
        {
            return true;
        }
        else
    ...
    public static TargetFramework Parse(ReadOnlySpan<char> text)
    {
        if (TryParse(text, out TargetFramework targetFramework))
        {
            return targetFramework;
        }
        else
        {
            throw new($"`{text}` is not a valid target framework");
        }
    }
